...
NSString * const BlobDownloadErrorDomain = @"com.thibaultcha.Blobdownload";
...
@property (nonatomic, assign, readwrite) BlobDownloadState state;
// Download
...
- (void)finishOperationWithState:(BlobDownloadState)state;
                   ...
                   delegate:(id<BlobDownloaderDelegate>)delegateOrNil
{
    ...
    if (self) {
        ...
        self.state = BlobDownloadStateReady;
...
- (void)start
{
    ...
    if (![NSURLConnection canHandleRequest:self.fileRequest]) {
        NSError *error = [NSError errorWithDomain:BlobDownloadErrorDomain
                                             code:BlobDownloadErrorInvalidURL
    ...
    if (self.connection && ![self isCancelled]) {
        ...
        self.state = BlobDownloadStateDownloading;
...
- (BOOL)isExecuting
{
    return self.state == BlobDownloadStateDownloading;
}

- (BOOL)isCancelled
{
    return self.state == BlobDownloadStateCancelled;
}

- (BOOL)isFinished
{
    return self.state == BlobDownloadStateCancelled || self.state == BlobDownloadStateDone || self.state == BlobDownloadStateFailed;
}
...
- (void)connection:(NSURLConnection*)connection didReceiveResponse:(NSURLResponse *)response
{
    ...
    if (httpResponse.statusCode >= 400) {
        error = [NSError errorWithDomain:BlobDownloadErrorDomain
                                    code:BlobDownloadErrorHTTPError
                                userInfo:@{ NSLocalizedDescriptionKey:[NSString stringWithFormat:@"Erroneous HTTP status code %ld (%@)",
                                                                       (long) httpResponse.statusCode,
                                                                       [NSHTTPURLResponse localizedStringForStatusCode:httpResponse.statusCode]],
                                            BlobDownloadErrorHTTPStatusKey: @(httpResponse.statusCode) }];
    }

    long long expected = @(self.expectedDataLength).longLongValue;
    if ([BlobDownloader freeDiskSpace].longLongValue < expected && expected != -1) {
        error = [NSError errorWithDomain:BlobDownloadErrorDomain
                                    code:BlobDownloadErrorNotEnoughFreeDiskSpace
...
- (void)cancel
{
    ...
    [self finishOperationWithState:BlobDownloadStateCancelled];
...
- (void)notifyFromCompletionWithError:(NSError *)error pathToFile:(NSString *)pathToFile
{
    ...
    BlobDownloadState finalState = success ? BlobDownloadStateDone : BlobDownloadStateFailed;
